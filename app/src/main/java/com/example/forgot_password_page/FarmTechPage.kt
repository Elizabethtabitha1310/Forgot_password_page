...
import com.example.forgot_password_page.Tab.Machineries
...
fun FarmTechPage(navController: NavController) {
    var selectedTab by remember { mutableStateOf<Tab>(Machineries(navController)) }
    ...
    Scaffold(
        ...
        Column(
            ...
            TabRow(
                selectedTabIndex = if (selectedTab is Machineries) 0 else 1,
                modifier = Modifier.padding(bottom = 16.dp)
            ) {
                Tab(
                    selected = selectedTab is Machineries,
                    onClick = { selectedTab = Machineries(navController) }
                ) {
            ...
            when (selectedTab) {
                is Machineries -> (selectedTab as Machineries).navController.MachineriesTabContent()
